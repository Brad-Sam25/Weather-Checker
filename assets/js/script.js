...
const history = (name) => {
    const requestUrl = `https://api.openweathermap.org/data/2.5/weather?q=${name}&appid=${apiKey}&units=imperial`;
    fetch(requestUrl)
        .then(response => {
            if(response.ok) {
                console.log('Success');
        ...
        .then(data1 => {
            ...
            let picData = document.querySelector("#today-icon");
            ...
            picData.innerHTML = `<img src="https://openweathermap.org/img/w/${data1.weather[0].icon}.png" />`;
